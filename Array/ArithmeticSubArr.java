...
public class ArithmeticSubArr {
    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        String[] str = br.readLine().trim().split(",");
        String[] str_l = br.readLine().trim().split(",");
        String[] str_h = br.readLine().trim().split(",");
        br.close();
        int[] arr = new int[str.length];
        int[] low = new int[str_l.length];
        int[] high = new int[str_h.length];
        for (int i = 0; i < str.length; i++) {
            arr[i] = Integer.parseInt(str[i]);
        }
        for (int i = 0; i < str_l.length; i++) {
            low[i] = Integer.parseInt(str_l[i]);
        }
        for (int i = 0; i < str_h.length; i++) {
            high[i] = Integer.parseInt(str_h[i]);
        }
        List<Boolean> result = checkArithmeticSubarrays(arr, low, high);
